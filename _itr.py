...
def get_tickets(filter):
    ...
    if(resp.status_code != 200): # response is not OK, throw error
        ...
        # filter tickets to just be the incident number and title as a string in an array
...
def get_client_info(url):
    ...
    if(resp.status_code != 200): # response not OK, print response and exit
        ...
        except KeyError: # if it fails (no response or no result), pass and return None
            pass
        return user_test
...
def get_tickets_in_progress():
    ...
    for ticket in tickets:
        ...
        elif recent[0]['sys_created_by'] == client: # most recent comment is client? needs response
            in_progress.append(ticket)
    # convert tickets into just strings with incident number and title
    in_progress = [ticket['number'] + ' ' + ticket['short_description'] for ticket in in_progress]
    return in_progress
...
# unassigned (priority max): new ticket or reassigned ticket from different department.
# Needs to be assigned to Stevenson or RCC and worked on
# client updated (priority secondary): client is the last responder on the ticket, needs response
...
def high_priority():
    ...
    # this removes exact duplicate tickets, but not those with different priorities
    ...
    ticket_no_dupes = {}
    for ticket in all_tickets: # go through all tickets
        # TODO check this if V
        if ticket[0] in ticket_no_dupes: # if ticket already processed
            if ticket[1] > ticket_no_dupes[ticket[0]]: # check if new ticket has higher priority
                ticket_no_dupes[ticket[0]] = ticket[1] # update to higher priority
        else: # if not in dict already, add it
            ticket_no_dupes[ticket[0]] = ticket[1]
    tickets_out = {'tickets':[]}
    # update formatting of tickets, key being the name of the ticket, value being it's priority
